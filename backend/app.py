...
def initialize_database():
    if not os.path.exists(DB_PATH):
        ...
        # Create showtime_includes_movie table
...
def save_movie_to_db_with_category(movie, category):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        INSERT OR IGNORE INTO movies (id, title, release_date, overview, vote_average, poster_path, category)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (
        movie['id'],
        movie['title'],
...
def fetch_movies_by_category(category_url, category_name):
...
def fetch_and_save_movies():
    ...
    fetch_movies_by_category(NOW_PLAYING_URL, "now_playing")

    print("Fetching Top Rated movies...")
    fetch_movies_by_category(TOP_RATED_URL, "top_rated")

    print("Fetching Upcoming movies...")
    fetch_movies_by_category(UPCOMING_URL, "upcoming")
...
def get_movies_by_category(category):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT id, title, release_date, overview, vote_average, poster_path
